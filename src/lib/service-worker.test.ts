...
describe("service worker cache lifecycle", () => {
  it("precaches the map route", async () => {
  ...
  it("removes only stale StudyMap caches for the previous deployment", async () => {
    const runtime = loadServiceWorker({
      cacheNames: [
        "app-old-build",
        "tiles-old-build",
    ...
    expect(runtime.caches.delete).toHaveBeenCalledWith("app-old-build");
    expect(runtime.caches.delete).toHaveBeenCalledWith("tiles-old-build");
