...
function checkFreshness({ today, dataDir }) {
  ...
  for (const file of files) {
    ...
    for (let index = 0; index < records.length; index++) {
      ...
      datedRecords++;
      const loc = `${file}[${index}] (id: ${record.id ?? "?"})`;

      if (!isRealIsoDate(record.valid_till)) {
        console.error(
          `ERROR ${loc}: valid_till must be a real ISO date (YYYY-MM-DD), got "${record.valid_till}"`,
        );
        totalErrors++;
        continue;
      }
      ...
      if (record.valid_till < today) {
        console.error(
          `ERROR ${loc}: valid_till expired on ${record.valid_till}; re-verify this record for ${today}`,
        );
        totalErrors++;
      }
    }
